...
struct hashid {
  ...
  int cap;
  int count;
  std::vector<hashid_node> id;
  std::vector<struct hashid_node *> hash;
};
...
static inline int hashid_full(struct hashid *hi) { return hi->count == hi->cap; }
